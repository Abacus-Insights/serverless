...
function removeTags() {
  const requests = this.apiGatewayUntagResourceParams.map(untagResourceParam => {
    // filter cloudformation tags that cannot be deleted ex) aws:cloudformation:stack-name
    console.log(untagResourceParam);
    if (untagResourceParam.tagKeys.length > 0) {
      untagResourceParam.tagKeys = untagResourceParam.tagKeys.filter(tag => !tag.startsWith("aws:"));
    }
    console.log(untagResourceParam);
